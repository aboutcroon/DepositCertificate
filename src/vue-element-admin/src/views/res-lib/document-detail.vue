        ...
        <el-row>
          <el-col :span="15">
            <el-tabs @tab-click="onchange" v-model="tabName" style="max-width: 500px;">
              <el-tab-pane label="开发指南" name="0" v-if="tabLis.dev">
              </el-tab-pane>
              <el-tab-pane label="接口管理" name="1" v-if="tabLis.api">
              </el-tab-pane>
              <el-tab-pane label="系统创建合约" name="2" v-if="tabLis.contractSys">
              </el-tab-pane>
              <el-tab-pane label="自建智能合约" name="3" v-if="tabLis.contractPer">
              </el-tab-pane>
            </el-tabs>
            <div class="file-preview">
              <div class="file-pre-title">{{docInfo.file_name}}</div>
              <div class="file-pre-time" v-if="docInfo.u_time !== '00：00'"><span>更新时间：</span><span>{{docInfo.u_time}}</span></div>
              <markdown-editor
                class="el-scrollbar"
                ref="markdownEditor"
          ...
          <el-col :span="9" class="divider-right">
            <div class="home-main-input" style="width: 100%">
              <Input v-model="keyword" style="border-color: #3B3783">
                <Button slot="append" style="height: 100%; color: #fff" @click="getFileList">搜索</Button>
              </Input>
            </div>
...
<script>
  ...
  export default{
    name: 'res_document_detail_nocache',
    components: {
      MarkdownEditor
    },
    data () {
    ...
    methods: {
      ...
      // 获取文件列表
      getFileList() {
        if (this.create_type === '系统创建') {
          this.tabLis.dev = true
          this.tabLis.api = true
          this.tabLis.contractSys = true
          this.tabLis.contractPer = true
        }
        const param = {
        ...
        this.http.apifiles(param).then(res => {
          if (res.status == '200') {
            ...
            if (newdata.length === 0) {
              ...
              this.$refs.markdownEditor.initEditor()
            }

            if (!this.$route.query.flag) {
              var len = newdata.length
              for (let i = 0; i < len; i ++) {
                if (newdata[i].name !== '交易确认查询接口' && newdata[i].name !== '合约调用' && newdata[i].name !== '合约测试文档' && newdata[i].name !== '存证接口' && newdata[i].name !== '交易详情查询接口' && newdata[i].name !== '获取用户token' && newdata[i].name !== '交易详情信息列表') {
                  delete newdata[i]
                }
              }
            }

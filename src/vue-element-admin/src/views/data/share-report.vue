        ...
        <el-col class="main-input-container" >
          ...
          <el-col class="normal-input">
            ...
            <Tooltip content="多个标签以英文的 , 隔开" placement="top-start" style="width:100%">
              <Input v-model="searchValue0" clearable placeholder="标签" class="search-input"  @keyup.enter.native="selectGet(1)"/>
            </Tooltip>
          </el-col>
          <el-col class="normal-input">
            <span class="icon iconfont iconIcon-search "></span>
          ...
          <el-col class="normal-input">
            <span class="icon iconfont iconIcon-search "></span>
            <Tooltip content="多个标签以英文的 , 隔开" placement="top-start" style="width:100%">
              <Input v-model="searchValue1" clearable placeholder="请输入共享用户" class="search-input"  @keyup.enter.native="selectGet(1)"/>
            </Tooltip>
          </el-col>
          <el-col class="normal-input time-input">
            <span class="icon iconfont icondata1"></span>
            <Date-picker v-model="times" type="daterange" placement="bottom-end" placeholder="选择操作时间" style="width:100%" @on-change="timechange"  @keyup.enter.native="selectGet(1)"/>
          </el-col>
        </el-col>
...
//            }
      ...
      tx(index, row) {
        ...
        this.$router.push({
          name: 'details', // 跳转的页面
          query: {
            'detailsdata': row.transaction_hash // 我们要传递的参数
          }
        })
      },
      jump(index, row) {
        this.$router.push({
          name: 'block', // 跳转的页面
          query: {
            'blockdata': row.block_number_int // 我们要传递的参数
          }
        })
      }
    }

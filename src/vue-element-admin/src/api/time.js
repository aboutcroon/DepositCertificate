export default {
  ...
  timestampToTime (time,type,num) {
    if(time){
      ...
      let mm = date.getMonth()<9? '0'+(date.getMonth()+1) : date.getMonth()+1;//月
  ...
  timeDifference(time){
    ...
    }else if(60<=difference&&difference<3600){//没达到1小时
      return Math.floor(difference/60)+'分钟'
    }else if(3600<=difference&&difference<86400){//没达到1天
      return Math.floor(difference/60/60)+'小时'
    }else if(86400<=difference){//超过或等于1天
